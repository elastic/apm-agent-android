...
Please note that when building an InstrumentationConfiguration object using `InstrumentationConfiguration.builder()`
all the instrumentations are disabled by default, so you only need to enable the ones you need using the builder setter methods.
...
Some of the runtime configurable options are available through Kibana's https://www.elastic.co/guide/en/kibana/master/agent-configuration.html[central configuration].

You can take a look at the available options https://github.com/elastic/apm/blob/main/specs/agents/mobile/configuration.md[here].