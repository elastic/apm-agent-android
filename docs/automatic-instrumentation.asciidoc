...
link:manual-instrumentation.asciidoc[manual instrumentation].
...
[arabic]
. Choose a link:#supported-instrumentations[supported instrumentation].
. Add its Gradle plugin to your project in the same location where the
link:getting-started.asciidoc#gradle-setup[agent] is added.
. link:getting-started.asciidoc#agent-setup[Initialize the agent] the same way
...
link:#configuration[a way to exclude] specific build types in your app
...
link:#compilation-behavior[compilation behavior] by excluding build
...
link:#supported-instrumentations[automatic instrumentations] that need
