...
class ConnectivityConfigurationTest {
    ...
    fun `Provide connectivity values`() {
        agentRule.initialize(connectivityInterceptor = {
            Connectivity.withSecretToken(
                "https://sample",
                "someSecretToken"
            )
        })

        assertThat(configuration.endpoint).isEqualTo("https://sample")
        assertThat(
            configuration.authConfiguration.asAuthorizationHeaderValue()
        ).isEqualTo("Bearer someSecretToken")
    }
