...
Sessions are currently time-based and will kept alive for at least 30 mins, a `session.id` attribute will be sent on every signal until the timer ends, resetting the timer whenever a new signal is created.
...
The values set through the Central configuration will override the ones set programmatically.
...
class MyApp extends android.app.Application {
    ...
    public void onCreate() {
        super.onCreate();
        ElasticApmConfiguration configuration = ElasticApmConfiguration.builder()
                .setSampleRate(0.5) // Only values between 0 and 1.
                .build();
        ElasticApmAgent.initialize(this, configuration);
    }
}
