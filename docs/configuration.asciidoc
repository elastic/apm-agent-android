...
class MyApp extends android.app.Application {
    ...
    public void onCreate() {
        ...
        PersistenceConfiguration persistenceConfiguration = PersistenceConfiguration.builder()
                ...
                .setMaxCacheSize(60 * 1024 * 1024) // 60 MB by default
                .setExportScheduler(ExportScheduler.getDefault(60 * 60 * 1000)) // One hour by default. The default ExportScheduler will run only when the host app is running, though you can create your own implementation of it in order to provide a better suited scheduler for your app.
                .build();
