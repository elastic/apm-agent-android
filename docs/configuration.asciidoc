...
Sessions are currently time-based for a minimum time of 30 mins, a `session.id` attribute will be sent on every signal until the timer ends, resetting the timer whenever a new signal is created.
