...
EDOT Android can upload the R8 mapping generated for an optimized application build to {{es}}. The mapping and the application's telemetry share the same build ID, allowing obfuscated crash stack traces to be associated with the correct build artifacts.

Viewing deobfuscated stack traces in {{kib}} requires version 1.0.0 or later of the [Android OpenTelemetry Assets](https://www.elastic.co/docs/reference/integrations/otel_android_dashboards) integration. Refer to the integration page for the minimum supported {{kib}} versions.

Refer to [Crash reporting](crash-reporting.md#deobfuscate-r8-stack-traces) to configure the mapping plugin and trigger uploads from your release workflow.
