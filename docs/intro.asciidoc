...
The agent attaches <<intro-features-session,session>> information to each span and
...
also provides <<manual-instrumentation,direct access>> to its
...
For <<distributed-tracing,distributed tracing>> to work properly,
...
finished. More info on the <<configuration,configuration>> page.
